...
public class PIDController {
	...
	private double m_totalError = 0.0; // the sum of the errors for use in the
										// integral calc
	private double m_tolerance = 0.05; // the percetage error that is considered
										// on target
	...
	private void calculate() {
		...
		if (m_enabled) {
			...
			/*
			 * Integrate the errors as long as the upcoming integrator does not
			 * exceed the minimum and maximum output thresholds
			 */
			...
			/*
			 * proportional action Le régulateur proportionnel (P : première
			 * règle) La commande de ce régulateur est proportionnelle à
			 * l'erreur. commande = Kp * erreur Kp est le coefficient de
			 * proportionnalité de l'erreur à régler de façon manuelle
			 */
			/*
			 * integral action Le régulateur proportionnel intégral (PI :
			 * première et seconde règle) La commande de ce régulateur est
			 * proportionnelle à l'erreur, mais aussi proportionnelle à
			 * l'intégrale de l'erreur. On rajoute donc à la commande généré par
			 * le régulateur proportionnel, la somme des erreurs commises au
			 * cours du temps. commande = Kp * erreur + Ki * somme_erreurs Ki
			 * est le coefficient de proportionnalité de la somme des erreurs.
			 * Il faut aussi le régler de façon manuelle.
			 */
			/*
			 * derivative action Le régulateur proportionnel dérivé (PD :
			 * première et troisième règle) La commande de ce régulateur est
			 * proportionnelle à l'erreur, mais aussi proportionnelle à la
			 * dérivée de l'erreur. La dérivée de l'erreur correspond à la
			 * variation de l'erreur d'un échantillon à l'autre et se calcule
			 * simplement en faisant la différence entre l'erreur courante et
			 * l'erreur précédente (c'est une approximation linéaire et locale
			 * de la dérivée). commande = Kp * erreur + Kd * (erreur -
			 * erreur_précédente) Kd est le coefficient de proportionnalité de
			 * la variation de l'erreur. Il faut régler ce coefficient
			 * manuellement.
			 */
			/*
			 * PID output Le régulateur proportionnel intégrale dérivé (PID :
			 * première, seconde et troisième rèlge) Ici, la commande est à la
			 * fois proportionnelle à l'erreur, proportionnelle à la somme des
			 * erreurs et proportionnelle à la variation de l'erreur. commande =
			 * Kp * erreur + Ki * somme_erreurs + Kd * (erreur -
			 * erreur_précédente)
			 */
